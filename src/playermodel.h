 ...
 * Used to determine which spot the player will be displayed in and able to control
 */
enum PlayerPosition {
    NONE = 0,
    LOWER_LEFT = 1,
...
// Gameplay - Cool Down Calc
    ...
    /// The number of times the space bar has been pressed since the cooldown timer has reset
     ...
     * Used to prevent user from pressing space faster than the animation can complete
     */
     ...
     * @todo these will need to be enabled at the start of gameplay. No need to run timers in the menus
     */
     ...
     * - !animationLocked
     * @return True if the player is allowed to lunge for food (Timers aren't blocking)
     */
    bool allowedToLunge();
...
// Gameplay - Timers
     ...
     * 
     * @details will increment the appropriate areas to keep track of cool down and animation timers
     * - Increment numPressesSinceTimeReset
     * - Check if allowedTolunge()
     * - Start the Animation timer
     * @return true if the player is allowed to lunge and successfully lunges else false
     */
    bool spacePressed();

private slots:
    /**
     * Should be called once the animation timer is reset to clear the bool bit appropriately
     * - Set animationLocked = false
     */
    void animationDone();

    /**
     * Should be called to keep reset the number of spaces since the last cool down period
