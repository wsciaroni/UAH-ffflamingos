 ...
 * @details Uses a Blocking Read to wait for all the information to arrive over the QTcpSocket and allows that to be accessed as a stream
 */
class BlockReader
{
public:
    /**
     * @brief Initializes a BlockReader on the specified IO device. In our case, we are using a QTcpSocket as our IO device.
...
private:
     ...
     * Blocking reads data from socket until buffer size becomes exactly n. No additional data is read from the socket.
     * @warning This can lock in an infinite loop if you do not read the correct amount of data.
     */
