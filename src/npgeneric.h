 ...
 * @details Used to determine what type of network packet the info is.  Allows
 * for a packet's role to be determined
 */
enum class PacketType : qint32 {
  NULLPACKETTYPE = 0,   ///< Should never occur
  PROVIDEROOMCODE = 1,  ///< Guest->Host Providing Room Code
  ROOMCODESTATUS =
      2,  ///< Host->Guest Letting them know if the room code was correct
  WELCOMETOROOM =
      3,  ///< Host->Guest Letting them know their UID and Position etc
 ...
 * @details Used to be passed from host->guest or guest->host providing the
 * necessary information
 */
class NPGeneric {
 private:
  /**
   * Used to determine the role of the packet and how it will be handled (As
   * well as the intended recipients)
   */
